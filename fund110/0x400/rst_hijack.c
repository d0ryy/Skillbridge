...
struct data_pass {	//used to pass data through the libpcap callback.
   int libnet_handle;
   u_char *packet;
}; 
...
as root.\n");
   ...
   pcap_loop(pcap_handle, -1, caught_packet, (u_char *)&critical_libnet_data); 	//user pointer. passed directly to callback func.
